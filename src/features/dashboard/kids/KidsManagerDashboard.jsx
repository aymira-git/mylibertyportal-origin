...
export default function KidsManagerDashboard() {
  ...
  // Scoped datasets strictly for Kindergarten
  ...
  const applications = useMemo(() => {
    return rawApplications.filter((a) =>
      matchesDivisionFilter(a.division || divisionOfProgram(a.programId || a.program), "kindergarten")
    );
  }, [rawApplications]);
  ...
  const classesWithIssues = useMemo(() => {
    ...
    return classes
      .map((c) => {
        const assignedUser = c.instructorId ? rawUsers.find((u) => u.id === c.instructorId) : null;
        const needsInstructor = !c.instructorId || !assignedUser;
        const instructorInactive =
          Boolean(c.instructorId) && assignedUser && !activeInstructorIds.has(c.instructorId);
  ...
  const activeShifts = useMemo(() => {
    return shifts.filter((s) => {
      const user = rawUsers.find((u) => u.id === s.userId);
      return (
        getShiftStatus(s) === "on_duty" &&
        user &&
        matchesDivisionFilter(user.division, "kindergarten")
      );
    });
  }, [shifts, rawUsers]);
