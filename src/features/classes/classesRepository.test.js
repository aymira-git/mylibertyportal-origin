...
const source = {
  ...
  classLevel: "warrior",
  studentIds: ["s1", "s2"],
  enrollments: [{ studentId: "s1", level: "warrior" }, { studentId: "s2", level: "warrior" }],
};
...
describe("transferStudentBetweenClasses", () => {
  ...
  it("updates the student's own level to the target class level after the batch lands", async () => {
    await transferStudentBetweenClasses({ sourceClass: source, targetClassId: "B", targetClass: target, studentId: "s1" });
    await vi.waitFor(() => expect(fake.find("users/s1")).toMatchObject({ data: { currentLevel: "elite" } }));
  });

  it("leaves transferReason out when it is blank", async () => {
    await transferStudentBetweenClasses({
      sourceClass: source, targetClassId: "B", targetClass: target, studentId: "s1", transferReason: "   ",
    });
  ...
  it.each([
    ...
    await transferStudentBetweenClasses({ sourceClass: source, targetClassId: "B", targetClass, studentId: "s1", ...extra });
  ...
  it("falls back to 'warrior' when no level is known anywhere", async () => {
    await transferStudentBetweenClasses({
      sourceClass: { id: "A", studentIds: ["s1"], enrollments: [] }, targetClassId: "B", targetClass: { id: "B" }, studentId: "s1",
    });
  ...
  it("writes nothing when the batch fails, and does not touch the student's level", async () => {
    ...
    await expect(
      transferStudentBetweenClasses({ sourceClass: source, targetClassId: "B", targetClass: target, studentId: "s1" })
...
describe("addStudentToClass / removeStudentFromClass", () => {
  it("adds a student and an enrollment record using arrayUnion", async () => {
    ...
    expect(op.data.enrollments.items[0]).toEqual({ studentId: "s9", dateJoined: "2026-09-21", level: "elite" });
...
describe("setClassGroupLevel", () => {
  it("sets the level on every class in the group and on each enrolment", async () => {
    await setClassGroupLevel([source, { id: "C", enrollments: [{ studentId: "s5", level: "warrior" }] }], "master");
...
describe("syncStudentsCurrentLevel", () => {
  it("updates each student and swallows a failure for one of them", async () => {
    ...
    expect(fake.opsOf("update").map((o) => o.path).sort()).toEqual(["users/s1", "users/s3"]);
  });
});
