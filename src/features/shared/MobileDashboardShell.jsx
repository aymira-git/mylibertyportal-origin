  ...
  return (
    <div className="md:hidden">
      <div className="sticky top-0 z-30 -mx-4 -mt-4 mb-4 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur sm:-mx-6 sm:-mt-6 sm:px-6">
        ...
        <h2 className="mt-0.5 text-lg font-black text-slate-800">{getCleanLabel(active?.label) || "Dashboard"}</h2>
              ...
              {moreTabs.map(tab => {
                ...
                return (
                  <button
                    ...
                    className={`min-h-14 rounded-xl border px-3 py-2 text-left text-xs font-bold transition flex items-center justify-between gap-2.5 ${
                      isActive
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 flex gap-1 bg-[#1a3a8f] px-1.5 pb-[max(0.4rem,env(safe-area-inset-bottom))] pt-1.5 shadow-[0_-8px_24px_rgba(15,23,42,0.25)]">
        {primaryTabs.map(tab => {
          const Icon = getTabIcon(tab);
          const label = getCleanLabel(tab.label);
          const isActive = activeTab === tab.id;
          ...
          return (
            <button
              key={tab.id}
              onClick={() => selectTab(tab.id)}
              className={`relative min-h-14 min-w-0 flex-1 rounded-xl px-1 flex flex-col items-center justify-center text-[10px] font-extrabold leading-tight transition ${
                isActive ? "bg-white text-[#1a3a8f] shadow-xs" : "text-white/75 active:bg-white/10"
              }`}
            >
        ...
        {moreTabs.length > 0 && (
          <button
            ...
            className={`min-h-14 min-w-0 flex-1 rounded-xl px-1 text-center text-[10px] font-extrabold transition ${
              activeIsInMore || moreOpen ? "bg-white text-[#1a3a8f] shadow-sm" : "text-white/75 active:bg-white/10"
            }`}
          >
