...
describe("approveApplication", () => {
  ...
  it("enrols the student in the chosen batch, and the batch level wins", async () => {
    ...
    expect(cls.data.enrollments.items[0]).toEqual({
      studentId: "auto-1",
  ...
  it.each([
    ...
    await expect(approveApplication({ app, classId: "c1" })).rejects.toThrow(
      "full or no longer open"
    );
    expect(fake.ops).toHaveLength(0);
  });

  it("refuses a batch that does not exist", async () => {
    await expect(approveApplication({ app, classId: "missing" })).rejects.toThrow(
      "full or no longer open"
    );
    expect(fake.ops).toHaveLength(0);
  });
...
describe("restoreApplication / deleteApplicationPermanently", () => {
  it("sets the application back to pending and removes the rejection fields", async () => {
    ...
    ["rejectedAt", "rejectedBy", "rejectedReason", "rejectedNote"].forEach((k) =>
      expect(data[k]).toEqual({ __op: "deleteField" })
    );
  });
