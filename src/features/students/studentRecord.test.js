...
describe("buildStudentRecord", () => {
  it("fills safe defaults for an empty form", () => {
    ...
    expect(r).toMatchObject({
      displayName: "", currentLevel: "warrior", rating: "1", paymentPlan: "monthly", status: "active", role: "student",
      gender: "", dob: "", joinedDate: "",
    });
  });

  it("trims text fields and stringifies numbers", () => {
    const r = buildStudentRecord({ displayName: "  Budi  ", phone: 81234567890, address: "\n Jl. Sam Ratulangi " });
  ...
  it("takes parent name and phone from the father, then the mother, then the legacy field", () => {
    ...
    expect(buildStudentRecord({ parentName: "Old", parentPhone: "0811" })).toMatchObject({ parentName: "Old", parentPhone: "0811" });
    expect(buildStudentRecord({ fatherPhone: "0811", motherPhone: "0822" }).parentPhone).toBe("0811");
  });
  ...
  it("keeps a supplied level, plan, status and join date", () => {
    const r = buildStudentRecord({ currentLevel: "epic", paymentPlan: "annual", status: "on_leave", joinedDate: "2019-06-15", rating: "4" });
    expect(r).toMatchObject({ currentLevel: "epic", paymentPlan: "annual", status: "on_leave", joinedDate: "2019-06-15", rating: "4" });
  });
...
describe("STUDENT_STATUS_OPTIONS", () => {
  it("lists the four student statuses", () => {
    expect(STUDENT_STATUS_OPTIONS.map((o) => o.value)).toEqual(["active", "on_leave", "graduated", "inactive"]);
  });
});
