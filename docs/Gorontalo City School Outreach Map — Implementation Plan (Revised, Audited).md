...
## Audit Notes (second pass, Claude)

> Checked this revision against the same uploaded `myliberty-portal.zip` used for the first audit. This is a second AI pass on top of a first AI pass on top of a third AI's plan — all three can still be wrong, worth a skim before treating any of it as settled.

**Architecture checks out.** `schoolOutreachRepository.js` pattern, Zod schema layer, `corporateEvents`-style camelCase naming, the immutable-`createdBy`/soft-delete Firestore rule shape, and `dateWita.js` usage all match real, existing code in the zip (e.g. `src/schemas/corporateEventSchema.js` and the `corporateEvents` block in `firestore.rules` already do exactly what this plan proposes for `schoolOutreach`). The visit-subcollection + batch-write design is a genuine improvement over the original plan's single-document approach.

**One correction to this plan's own "correction"**: it says the earlier audited version claimed `package.json` was missing `test`/`typecheck` scripts. That's not something the earlier audit said — neither the original plan nor the audit made that claim (the original plan's own verification section already assumed `npm test`/`npm run typecheck` existed). Not a big deal either way since the end result is the same, but worth noting since it's misattributed to a version that didn't say it.

**Small thing to keep in mind, not a blocker**: this revision says it checked against the live `main` branch on GitHub rather than the uploaded zip. Everything checked against the zip lines up, so this is likely just the zip and `main` being in sync — but if a fresh zip is ever pulled before building this, a quick diff wouldn't hurt in case something moved between the two.

**Tile-provider abstraction (section 9)** — making the OSM tile URL configurable is cheap and reasonable to keep. Worth knowing this is a "nice to have now, matters more later" item: at the scale of one marketing officer visiting schools in one city, actual OSM tile-usage-policy limits are unlikely to be hit any time soon. Not a reason to drop it, just not urgent if time is tight.

Everything else in this plan (open decisions in section 15, the map/UI structure, the security rules, the implementation order) looks sound and ready to build from.
